...
function MyListMovie() {
  ...
  return (
    <section>
      <ModalWatchMovie modalWatchMovie={modalWatchMovie} closeModalWatchMovie={closeModalWatchMovie} dataMovie={dataMovie} fetchTransaction={fetchTransaction} />
      <section className='w-full mt-20 px-4 md:px-10 lg:px-20 pb-10'>
          <p className='w-full mb-5 font-bold text-2xl text-[#D2D2D2]'>My List Movie</p> 
          <div className='grid grid-cols-4 gap-3 max-md:grid-cols-2 max-sm:grid-cols-1'>
            {transactions?.map((transaction, i) => {
              {if(transaction?.status === 'approved') {
                return (
                  <div key={i} className='rounded-md overflow-hidden shadow-md shadow-gray-700'>
                        <div className='w-full h-48'>
                          <Image src={transaction?.movie?.thumbnail} alt={transaction?.movie?.title} width={400} height={400} className='w-full h-full object-cover' />
                        </div>
    
                        <div className='p-5'>
                          <p className='mb-2 text-xl font-bold tracking-tight text-[#D2D2D2] dark:text-[#D2D2D2]'>{transaction?.movie?.title}</p>
                          <button type='button' className='inline-flex items-center px-3 py-2 text-sm font-medium bg-[#3E3E3E] text-center text-[#D2D2D2] rounded-md hover:opacity-80' onClick={() => {setModalWatchMovie(true); setDataMovie(transaction?.movie)}}>Watch movie
