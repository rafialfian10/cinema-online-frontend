...
export default function Navbar() {
    ...
    const handleLogout = async (e: any) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                toast.success('Logout Successfully!', { position: 'top-right', autoClose: 2000, hideProgressBar: false, closeOnClick: true, pauseOnHover: false,draggable: true, progress: undefined, theme: 'colored', style: {marginTop: '65px'}});
                router.push('/');
                signOut();
            } else {
                Swal.close();
    ...
    return (
        <Fragment>
                            ...
                            {status === 'unauthenticated' ? (
                                <div className='hidden sm:ml-6 sm:block'>
                                    <div className='flex space-x-4'>
                                        <button type='button' className='h-10 w-100 flex justify-center items-center rounded p-5 bg-[#3E3E3E] hover:opacity-80 text-[#D2D2D2]' onClick={openModalLogin}>Login</button>
                                        <button type='button' className='h-10 w-100 flex justify-center items-center rounded p-5 bg-[#CD2E71] hover:opacity-80 text-[#D2D2D2]' onClick={openModalRegister}>Register</button>
                                    </div>
                                </div>
                            ) : userAuth?.data?.role === 'user' ? (
                                <div className='hidden sm:ml-6 sm:block'>
                                    <div className='flex space-x-3'>
                                        <button type='button' className='relative w-7 h-7 flex justify-center items-center rounded-full bg-[#3E3E3E] text-[#D2D2D2] focus:outline-none focus:ring-1 focus:ring-[#D2D2D2] focus:ring-offset-1 focus:ring-offset-[#D2D2D2]'>
                                        ...
                                        <Menu as='div' className='relative'>
                                            <div>
                                                <Menu.Button className='w-10 h-10 relative flex rounded-full overflow-hidden'>
                                                    ...
                                                    {user?.photo && user?.photo !== 'http://localhost:5000/uploads/photo/' ? (
                                                        <Image src={user?.photo} alt={user?.username ?? 'User'} width={40} height={40} className='rounded-full' priority={true} />
                                                    ) : (
                                                        <Image src={defaultPhoto} alt={user?.username ?? 'User'} width={40} height={50} className='p-1 rounded-full' priority={true} />
                                                    )}
                                                </Menu.Button>
                                            </div>
                                            <Transition as={Fragment} enter='transition ease-out duration-100' enterFrom='transform opacity-0 scale-95' enterTo='transform opacity-100 scale-100' leave='transition ease-in duration-75' leaveFrom='transform opacity-100 scale-100' leaveTo='transform opacity-0 scale-95'>
                                                <Menu.Items className='absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md py-1 bg-[#0D0D0D] shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none'>
                                                    ...
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href='/pages/users/my-list-movie' className={classNames(active ? 'hover:bg-[#313131]' : '', 'block px-4 py-2 text-sm text-[#D2D2D2]')}>My Movie</Link>
                                                        )}
                                                    </Menu.Item>
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <button type='button' className={classNames(active ? 'hover:bg-[#313131]' : '', 'w-full block px-4 py-2 text-start text-sm text-[#D2D2D2]')} onClick={handleLogout}>Logout</button>
                                                        )}
                                                    </Menu.Item>
                                                </Menu.Items>
                                            </Transition>
                                        ...
                                        <Menu as='div' className='relative'>
                                            <div>
                                                <Menu.Button className='w-10 h-10 relative flex rounded-full overflow-hidden'>
                                                    ...
                                                    <span className='sr-only'>Open user menu</span>
                                                    {user?.photo && user?.photo !== 'http://localhost:5000/uploads/photo/' ? (
                                                        <Image src={user?.photo} alt={user?.username ?? 'Admin'} width={40} height={40} className='rounded-full' priority={true} />
                                                    ) : (
                                                        <Image src={defaultPhoto} alt={user?.username ?? 'Admin'} width={40} height={40} priority={true} />
                                                    )}
                                                </Menu.Button>
                                            </div>
                                            <Transition as={Fragment} enter='transition ease-out duration-100' enterFrom='transform opacity-0 scale-95' enterTo='transform opacity-100 scale-100' leave='transition ease-in duration-75' leaveFrom='transform opacity-100 scale-100' leaveTo='transform opacity-0 scale-95'>
                                                <Menu.Items className='absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md py-1 bg-[#0D0D0D] shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none'>
                                                    ...
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href='/pages/admin/list-movie' className={classNames(active ? 'hover:bg-[#313131]' : '', 'block px-4 py-2 text-sm text-[#D2D2D2]')}>List Movie</Link>
                                                        )}
                                                    </Menu.Item>
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href='/pages/admin/list-category' className={classNames(active ? 'hover:bg-[#313131]' : '', 'block px-4 py-2 text-sm text-[#D2D2D2]')}>List Category</Link>
                                                        )}
                                                    </Menu.Item>
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <Link href='/pages/admin/list-transaction' className={classNames(active ? 'hover:bg-[#313131]' : '', 'block px-4 py-2 text-sm text-[#D2D2D2]')}>List Transaction</Link>
                                                        )}
                                                    </Menu.Item>
                                                    <Menu.Item>
                                                        {({ active }) => (
            ...
            <Register modalRegister={modalRegister} closeModalRegister={closeModalRegister} openModalLogin={openModalLogin} />
            <Login modalLogin={modalLogin} setModalLogin={setModalLogin} closeModalLogin={closeModalLogin} openModalRegister={openModalRegister} />
