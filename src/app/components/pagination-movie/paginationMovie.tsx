...
const PaginationMovie: FC<PaginationMovieProps> = ({
  totalMovies,
  firstPage,
  lastPage,
}) => {
      ...
      visiblePageNumbers.push(
        <li key={i}>
          <button
            ...
            onClick={() => router.push(`?page=${i}&per-page=${moviesPerPage}`)}
          >
            {i}
          </button>
        </li>
      ...
      <div className="mb-2 flex max-sm:justify-center">
        <p className="text-sm text-gray-700">
          ...
          <span className="font-medium">
            {Number(page) * Number(moviesPerPage)}{" "}
          </span>
          to{" "}
          <span className="font-medium">
            {" "}
          ...
          <ul className="h-10 flex items-center -space-x-px text-base">
            <li>
              <button
                ...
                onClick={() => router.push(`?page=1&per-page=${moviesPerPage}`)}
                disabled={!firstPage}
              >
            ...
            <li>
              <button
                ...
                onClick={() =>
                  router.push(
                    `?page=${Number(page) - 1}&per-page=${moviesPerPage}`
                  )
                }
                disabled={!firstPage}
              >
            ...
            <li>
              <button
                ...
                onClick={() =>
                  router.push(`?page=${totalPages}&per-page=${moviesPerPage}`)
                }
                disabled={!lastPage}
              >
                <span className="text-[#D2D2D2] font-bold">{`>>`}</span>
