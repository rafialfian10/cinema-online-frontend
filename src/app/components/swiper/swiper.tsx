...
export default function Swipers({ movies }: MoviesProps) {
                ...
                if(response.data.status === 200) {  
                    (window as any).snap.pay(response.data.data.token, {
                        onSuccess: function (result: any) {
                            ...
                            window.location.replace(`/pages/users/profile/${userCheckAuth?.id}`);
                        },
    ...
    return (
        <div className='w-full mx-auto mt-20 md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl'>
            <Swiper modules={[Navigation, Pagination, Autoplay]} slidesPerView={1} pagination={{clickable: true}} loop={true} autoplay={{delay: 3000, disableOnInteraction: false}} spaceBetween={30} centeredSlides={true} navigation className='max-sm:w-full' style={{maxWidth: '90vw'}}>
                            ...
                            <div className='w-2/3 max-md:w-full top-12 max-md:top-3 px-10 max-md:px-5 max-sm:px-10 absolute flex flex-col'>
                                ...
                                <div className='flex flex-wrap'>
                                    {movie?.category.map((category: any) => {
                                        return (
                                            <p key={category?.id} className='text-lg max-md:text-base max-sm:text-xs mr-2 font-bold text-[#D2D2D2]'>{category?.name},</p>
                                        );
                                    })}
                                </div>
                                <p className='mb-3 max-md:mb-2 text-lg max-md:text-base max-sm:text-xs font-bold text-[#CD2E71]'>Rp. {movie?.price.toLocaleString('id-ID', { minimumFractionDigits: 2 })}</p>
                                {windowWidth > 768 ? (
                                    <p className='mb-3 max-md:mb-1 text-base max-md:text-sm max-sm:text-xs text-[#D2D2D2]'>{movie?.description}</p>
                                ) : windowWidth > 640 ? (
